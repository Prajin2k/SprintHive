    ...
    <div className="relative" ref={searchRef}>
      {!isOpen ? (
        <button 
          ...
          className="w-10 h-10 rounded-full flex items-center justify-center text-surface-400 hover:text-surface-100 hover:bg-surface-800 transition-colors"
        >
          🔍
        </button>
      ) : (
        <div className="flex items-center bg-surface-800 rounded-full border border-brand-500/50 w-64 md:w-96 px-3 shadow-[0_0_15px_rgba(249,115,22,0.15)] transition-all duration-300">
          <span className="text-surface-400">🔍</span>
          <input
            autoFocus
            type="text"
            className="bg-transparent border-none focus:ring-0 text-surface-50 w-full py-2 px-3 text-sm placeholder-surface-500"
            placeholder="Search projects, tasks, people..."
            value={query}
            onChange={(e) => setQuery(e.target.value)}
          />
          {loading && <span className="text-brand-500 text-xs font-medium animate-pulse">...</span>}
        </div>
      )}

      {isOpen && query.trim() && (
        <div className="absolute top-14 right-0 md:left-0 md:right-auto w-80 md:w-96 bg-surface-800 rounded-xl shadow-2xl border border-surface-700 z-50 overflow-hidden max-h-[80vh] overflow-y-auto">
